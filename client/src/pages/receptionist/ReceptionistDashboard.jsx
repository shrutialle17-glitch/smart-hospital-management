...
const ReceptionistDashboard = () => {
  ...
  const { data: pendingBills, isLoading: isBillsLoading } = useQuery({
    queryKey: ["receptionBills"],
    queryFn: async () => {
      const { data } = await api.get("/billing", {
        params: { status: "UNPAID", limit: 10 },
      });
      return data.data;
    },
  });
  ...
  const payBillMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries(["receptionBills"]);
      toast.success("Payment recorded successfully!");
    },
    onError: (err) => {
      toast.error(
        err.response?.data?.error?.message || "Failed to record payment.",
      );
    },
  });
  ...
  const registerMutation = useMutation({
    ...
    onError: (err) => {
      toast.error(
        err.response?.data?.error?.message || "Failed to register patient.",
      );
    },
  });
  ...
  const handlePrintID = (patient) => {
    ...
    toast.success(
      `Printing ID Card for ${patient.user.firstName} ${patient.user.lastName}...`,
    );
  };
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-right flex justify-end gap-2">
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => handlePrintID(apt.patient)}
                          >
                            Print ID
                          </Button>
                          <Button
              ...
              {isBillsLoading ? (
                ...
                <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
                  {pendingBills.map((bill) => (
                    <div
                      key={bill.id}
                      className="p-3 bg-surface border border-gray-100 dark:border-gray-800 rounded-lg flex items-center justify-between shadow-sm"
                    >
                      <div>
                      ...
                      <Button
                        size="sm"
                        className="bg-primary/10 hover:bg-primary/20 text-primary border-none shadow-none"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="space-y-1">
                      ...
                      <select
                        required
                        className="w-full p-2 border border-gray-200 dark:border-gray-800 rounded-lg text-sm bg-surface"
                ...
                <div className="space-y-1">
                  ...
                  >
                    ...
                    {doctors?.map((doc) => (
                      <option
                        key={doc.id}
                        value={doc.doctorProfile?.id || doc.id}
                      >
